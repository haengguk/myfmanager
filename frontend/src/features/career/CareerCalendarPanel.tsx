...
export function CareerCalendarPanel({ calendar, loading, pending, competitionPending = false, error, onAdvance, onRefresh, onCompetitionAction, onReconcilePending = onRefresh, onAnalyzeOpponent, focusFixture, onClosePreparation }: {
  ...
  if (loading) return <section className="ca-calendar ca-calendar--loading" aria-label="Career 캘린더" aria-busy="true"><span>게임 날짜</span><strong>캘린더 확인 중…</strong></section>;
  if (!calendar) return <section className="ca-calendar ca-calendar--error" aria-label="Career 캘린더"><span>게임 날짜</span><strong>날짜 진행을 사용할 수 없습니다</strong><p>{error ?? '서버 캘린더 응답이 없습니다.'}</p><button type="button" className="lm-text-button" onClick={onRefresh}>캘린더 다시 확인</button></section>;
  ...
  const competitionActionLabel = competitionCommand === 'START_PLAYER_COMPETITION_SERIES' ? '관리 경기 직접 시작'
    : competitionCommand === 'RESUME_PLAYER_COMPETITION_SERIES' ? '진행 중인 관리 경기 계속'
  ...
  return <section className="ca-calendar" aria-label="Career 캘린더">
    ...
    {focusFixture ? <p role="status">{calendar.competition.nextFixture?.fixtureId === focusFixture ? "선택한 관리 경기의 준비 화면입니다. 기존 시작 버튼으로 직접 진행하세요." : "준비 대상 관리 경기 전에 다른 경기가 있습니다. 아래 실행 가능한 경기부터 진행하세요."}<button type="button" className="lm-text-button" onClick={onClosePreparation}>경기 준비 표시 닫기</button></p> : null}
    <header>
      <div><span>게임 날짜 / {calendar.activeCalendarSeasonYear}</span><strong><time dateTime={calendar.currentDate}>{calendar.currentDate}</time></strong><small>{current ? current.displayNameKo : '첫 공식 일정 이전'}{calendar.currentStage ? ` · ${calendar.currentStage.displayNameKo}` : calendar.nextStage ? ` · 다음 단계 ${calendar.nextStage.displayNameKo}` : ''}</small></div>
      <div className="ca-calendar__revision"><span>저장 갱신</span><strong>{calendar.calendarRevision}</strong><small>{calendar.projectionStatus === 'GAME_PROJECTED_FROM_2026_TEMPLATE' ? '2026 기준 투영' : '기준 연도'}</small></div>
    </header>
    {error && !calendar.activePendingAdvance ? <div role="status" aria-live="polite"><p>{error}</p><button type="button" className="lm-text-button" onClick={onRefresh}>경기 상태 다시 확인</button></div> : null}
    {calendar.activePendingAdvance ? <div className="ca-calendar__stop is-auto_fixtures_pending" role="status" aria-live="polite"><strong>서버 작업 복구 중</strong><span>{calendar.activePendingAdvance.mode === 'ADVANCE_ONE_DAY' ? '하루 진행' : '다음 일정 진행'} · 같은 원본 요청으로 처리 여부를 확인합니다.{error ? ` ${error}` : ''}</span><button type="button" className="lm-text-button" disabled={pending} onClick={onReconcilePending}>상태 다시 확인</button></div> : calendar.blockingReason ? <div className={`ca-calendar__stop is-${calendar.blockingReason.toLowerCase()}`} role="alert"><strong>{calendar.blockingReason === 'ROSTER_REPAIR_REQUIRED' ? '선수단 복구 필요' : calendar.blockingReason === 'SEASON_ROLLOVER_REQUIRED' ? '시즌 마감과 스토브 운영' : calendar.blockingReason}</strong><span>{calendar.blockingReason === 'ROSTER_REPAIR_REQUIRED' ? '계약·선발 또는 등록 자격이 부족합니다. 계약 시장에서 날짜를 진행하며 영입·승격·선발·보충등록으로 복구하세요. 미완료 경기는 유지됩니다.' : calendar.blockingReason === 'MANAGED_FIXTURE_REQUIRED' ? '관리 팀 경기를 League에서 완료해야 날짜가 지나갑니다.' : calendar.blockingReason === 'AUTO_FIXTURES_PENDING' ? '저장된 Auto 경기와 outbox 완료를 기다립니다.' : calendar.blockingReason === 'SEASON_PAUSED' ? 'League 시즌이 일시 정지되어 날짜를 진행하지 않습니다.' : calendar.blockingReason === 'SEASON_CANCELLED' ? '취소된 시즌에서는 날짜를 진행하지 않습니다.' : calendar.blockingReason === 'COMPETITION_TRANSITION_REQUIRED' ? '검증된 R1–R2 완료 결과가 아직 다음 대회 입력으로 봉인되지 않았습니다.' : calendar.blockingReason === 'LEGACY_PENDING_RECONCILIATION_REQUIRED' ? '이전 버전 작업은 원본 mode와 revision을 증명할 수 없어 새 날짜 진행을 차단합니다.' : calendar.blockingReason === 'MANAGED_COMPETITION_FIXTURE_REQUIRED' ? '관리 팀의 대회 Series 완료가 필요합니다.' : calendar.blockingReason === 'AUTO_COMPETITION_FIXTURE_REQUIRED' ? '대회 Auto Series 실행이 필요합니다.' : calendar.blockingReason === 'LCK_PLAYOFF_BRACKET_RULE_SOURCE_INCOMPLETE' ? '10경기 날짜는 확인됐지만 승자·패자 routing 근거가 부족해 대진을 생성하지 않습니다.' : calendar.blockingReason === 'SEASON_NOT_READY' ? 'League 시즌이 아직 실행 준비 상태가 아닙니다.' : calendar.blockingReason === 'SEASON_ROLLOVER_REQUIRED' ? calendar.allowedAdvanceModes.length ? '대회 마감 후 계약 시장에서 스토브를 운영하세요. 날짜를 12월 31일까지 진행한 뒤 다음 시즌을 시작할 수 있습니다.' : '시즌 마감 조건을 확인하고 다음 시즌 시작 버튼으로 진행하세요.' : '서버 권위 상태 확인이 필요합니다.'}</span></div> : null}
    {competition ? <div className="ca-calendar__competition" aria-label="Career 대회 상태">
      <span>{calendar.competition.currentCompetition ? '현재 대회' : '다음 대회'}</span>
      ...
      {calendar.competition.internationalCompetitions?.map(international => <details key={international.competitionId} className="ca-calendar__competition-data">
        ...
        <p>저장된 참가 자격과 대회 결과입니다. 경기 진행은 기존 Draft와 시뮬레이션을 사용합니다.</p>
      ...
      {calendar.competition.groupStandings.length ? <div className="ca-calendar__competition-data"><span>조별 순위</span><ol>{calendar.competition.groupStandings.map((standing) => <li key={`${standing.groupId}-${standing.teamCode}`}><b>{standing.groupId} {standing.groupRank}</b><strong>{standing.teamCode}</strong><small>{standing.matchWins}승 {standing.matchLosses}패 · {standing.gameWins - standing.gameLosses >= 0 ? '+' : ''}{standing.gameWins - standing.gameLosses} · 그룹 {standing.groupPoints}점</small></li>)}</ol></div> : null}
      {calendar.competition.currentSeeds.length ? <div className="ca-calendar__competition-seeds"><span>현재 시드</span>{calendar.competition.currentSeeds.map((seed) => <small key={`${seed.competitionId}-${seed.seedScope}-${seed.seedNumber}`}>{seed.seedScope} {seed.seedNumber} · <b>{seed.teamCode}</b></small>)}</div> : null}
    ...
    <div className="ca-calendar__grid">
      <div className="ca-calendar__events"><span>다가오는 일정</span><ol>{calendar.upcomingEvents.slice(0, 6).map((event) => <li key={event.eventId}><time>{range(event.startDate, event.endDate)}</time><div><strong>{event.displayNameKo}</strong><small>{calendar.projectionStatus === 'GAME_PROJECTED_FROM_2026_TEMPLATE' ? '게임 내 투영 일정' : STATUS_COPY[event.officialStatus] ?? event.officialStatus} · {EXECUTION_COPY[event.executionStatus]}</small></div></li>)}</ol></div>
      <div className="ca-calendar__fixtures"><span>정규리그 R1–R2 일정</span>{calendar.upcomingFixtures.length ? <ol>{calendar.upcomingFixtures.slice(0, 4).map((fixture) => <li key={fixture.fixtureId}><time>{fixture.date}</time><strong>R{fixture.roundNumber} · {fixture.firstTeamCode} vs {fixture.secondTeamCode}</strong><small>{fixture.executionMode === 'PLAYER_CONTROLLED' ? '관리 경기' : fixture.jobStatus ?? fixture.lifecycleStatus}</small></li>)}</ol> : <p>남은 정규리그 R1–R2 경기가 없습니다. 현재 진행할 대회는 위에서 확인하세요.</p>}<footer><span>공식 미정 {calendar.pendingOfficialFields.length}</span><span>KeSPA Cup · 2025 참고 규칙 · 2026 공식 규칙/외부 참가팀 미확정</span></footer></div>
    </div>
  </section>;
}
