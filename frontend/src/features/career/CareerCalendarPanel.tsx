...
export function CareerCalendarPanel({ calendar, loading, pending, competitionPending = false, error, onAdvance, onRefresh, onCompetitionAction, onReconcilePending = onRefresh, onAnalyzeOpponent, focusFixture, onClosePreparation }: {
  ...
  return <section className="ca-calendar" aria-label="Career 캘린더">
    ...
    <div className="ca-calendar__grid">
      <div className="ca-calendar__events"><span>UPCOMING / OFFICIAL + PROJECTED</span><ol>{calendar.upcomingEvents.slice(0, 6).map((event) => <li key={event.eventId}><time>{range(event.startDate, event.endDate)}</time><div><strong>{event.displayNameKo}</strong><small>{calendar.projectionStatus === 'GAME_PROJECTED_FROM_2026_TEMPLATE' ? '게임 내 투영 일정' : STATUS_COPY[event.officialStatus] ?? event.officialStatus} · {EXECUTION_COPY[event.executionStatus]}</small></div></li>)}</ol></div>
