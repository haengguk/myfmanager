...
export function CareerPlayerPromises({ market, playerId }: { market: CareerMarket | null; playerId: string }) {
  ...
  return <div aria-label="출전·약속·만족도"><h4>출전과 역할 약속</h4><p>게임 내 평가입니다. 능력치·Draft·승패에는 영향을 주지 않습니다. 28일·평가 기회 6 Series를 관찰하고 14일 간격으로 평가합니다.</p>
    ...
    {loans.map(l => <p key={l.loanId}><strong>{l.status === 'ACTIVE' ? '임대 중' : '임대 종료'}</strong> · 원소속 {l.parentTeam} / 임대 출전 구단 {l.borrowingTeam} · {l.startDate} ~ {l.endDate} (복귀일 {returnDate(l.endDate)})<br />임대료 {l.fee.toLocaleString('ko-KR')} 크레딧 · 급여 원소속 {100 - l.borrowerSalaryPercent}% / 임대 구단 {l.borrowerSalaryPercent}%</p>)}
