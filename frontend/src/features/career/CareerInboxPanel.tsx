...
export function CareerInboxPanel({ careerId, year, revision, running, busy, onNavigate }: { careerId: string; year: number; revision: number; running: boolean; busy: boolean; onNavigate: (link: InboxLink) => void }) {
  ...
  return <section className="ca-calendar ca-inbox" aria-label="Career 소식함">
    <header><div><span>소식과 현재 업무</span><strong>처리 필요 {decisions.length}건 · 소식 {feed?.unread ?? 0}건 미읽음</strong></div><button onClick={() => { setPage({ cursor: 0, asOf: null }); setRefresh(v => v + 1); }}>소식 새로고침</button></header>
    <p>읽음은 확인 표시입니다. 실제 계약·등록·경기 처리는 연결된 화면에서 직접 실행하며, 연속 진행 재개도 직접 선택합니다.</p>
    {error ? <p role="alert">{error}</p> : null}
    <h3>현재 처리 필요</h3>{!feed ? <p role="status">최신 업무를 확인하고 있습니다.</p> : null}{decisions.length ? decisions.map(d => <article key={`${d.id}:${d.revision}`}><strong>{d.title}</strong><p>{d.date} · 미해결 · {d.blocksProgress ? '진행 대기' : '확인용'} · 기한 {d.deadline ?? '별도 기한 없음'}</p><p>{d.summary}</p><button disabled={pending || busy} onClick={() => navigate(d)}>해당 업무로 이동</button></article>) : feed ? <p>현재 사용자 응답이 필요한 업무가 없습니다. 캘린더·연속 진행에서 허용된 진행 방법을 선택하세요.</p> : null}
