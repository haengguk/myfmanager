import { focusPlayTarget } from './careerPlayFlow';
import { pricingLabel } from './api/careerPricing';
...
export function CareerMarketPanel({ careerId, year, revision, historical, busy, focusPlayer, focus, onBegin, onChanged, section = 'market', onInbox, onCloseDetail }: {
  section?: 'market' | 'finance'; onInbox?: () => void; onCloseDetail?: () => void;
  ...
  useEffect(() => { setNotice(''); setLegacyRefresh(false); setPending(false); setSelected(null); setEditing(null); setReleaseReview(false); setView(null); setRoster(null); return () => { mutation.current?.controller.abort(); mutation.current?.release(); mutation.current = null; }; }, [careerId, year]);
  useEffect(() => { if (closingDetail.current && view && loadedFocus.current === focus && !selected) { closingDetail.current = false; focusPlayTarget('#career-contract-market'); } }, [view, focus, selected]);
  const money = (n: number) => careerMoney(n, view?.currency ?? 'GAME_CREDITS');
  const pick = (id: string, offer?: MarketOffer) => {
    const p = view?.players.find(p => p.playerId === id); window.sessionStorage.setItem(`career-player:market:${careerId}:${year}`, JSON.stringify({ id, offer: offer?.offerId ?? null })); setSelected(id); setEditing(offer?.offerId ?? null); setReleaseReview(false); setReplacement('');
  ...
  useEffect(() => {
    if (!view || selected || focusPlayer || focus?.playerId) return;
    try { const saved = JSON.parse(window.sessionStorage.getItem(`career-player:market:${careerId}:${year}`) ?? 'null'); if (saved && view.players.some(p => p.playerId === saved.id)) { pick(saved.id, view.offers.find(o => o.offerId === saved.offer && o.playerId === saved.id)); setFilter('all'); } } catch { /* Optional navigation state is not a command. */ }
  }, [view?.careerId, view?.seasonYear]);
  useEffect(() => { if (view && section === 'finance' && focus?.panel === 'FINANCE') focusPlayTarget('[aria-label="Career 재정"]'); }, [view, section, focus]);
  useEffect(() => { if (view && selected && section === 'market') focusPlayTarget(focus?.panel === 'TRADE' ? '[aria-label="유료 이적과 임대"]' : '#career-market-player'); }, [selected, view?.careerId, focus, section]);
  const execute = async (action?: MarketCommand['action'], offerId?: string) => {
    ...
    try {
      ...
      window.sessionStorage.removeItem(marketOperationKey(careerId)); setOperation(null); setView(old => newerMarket(old, result.market)); setEditing(null); setReleaseReview(false); setNotice(['SUBMIT', 'REVISE'].includes(body.action) ? '계약 조건을 접수했습니다. 선수 판단과 효력일 적용은 아래 원본 제안에서 확인하세요.' : '요청의 저장 결과를 확인했습니다. 최신 현재 업무에서 다음 행동을 선택하세요.');
  ...
  if (section === 'finance') return <section className="ca-calendar" aria-label="Career 재정"><h2>구단 재정</h2>{budget ? <dl className="ca-home-facts"><div><dt>구단 현금</dt><dd>{money(budget.cash)}</dd></div><div><dt>지급 여유</dt><dd>{budget.paymentHeadroom === undefined ? '미제공' : money(budget.paymentHeadroom)}</dd></div><div><dt>연봉 예산</dt><dd>{money(budget.annualBudget)}</dd></div><div><dt>현재 연봉 총액</dt><dd>{money(budget.currentAnnualSalary)}</dd></div><div><dt>예약 현금</dt><dd>{money(budget.reservedCash)}</dd></div><div><dt>확정 최대 연봉</dt><dd>{money(budget.committedPeakSalary)}</dd></div></dl> : <p>구단 재정 미제공</p>}<p>예약 금액은 기존 약정입니다. 예산 한도와 지급 여유는 구단 현금과 구분합니다.</p><CareerFinancePanel market={view} />{!view.clubFinance ? <p>이 저장은 시즌 목표·평가 자료를 제공하지 않습니다.</p> : null}</section>;
  return <section id="career-contract-market" className="ca-calendar" aria-label="계약과 FA 시장" aria-busy={pending}>
    ...
    {budget ? <details><summary>계약 예산 · 예약 확인</summary><dl><dt>연봉 예산 / 기간별 최대 약정·제안</dt><dd>{money(budget.annualBudget)} / {money(budget.committedPeakSalary)}</dd><dt>잔액 / 제안 계약금 예약</dt><dd>{money(budget.cash)} / {money(budget.reservedCash)}</dd><dt>현재 연봉 부담</dt><dd>{money(budget.currentAnnualSalary)}</dd><dt>현재 약정의 급여 확보 후 추가 지급 여유</dt><dd>{budget.paymentHeadroom === undefined ? '미제공' : money(budget.paymentHeadroom)}</dd><dt>미지급 급여</dt><dd>{budget.salaryArrears === undefined ? '미제공' : money(budget.salaryArrears)}</dd></dl></details> : null}
    {notice ? <p role="status">{notice}</p> : null}{focus && ['MARKET', 'TRADE', 'FINANCE'].includes(focus.panel) ? <button type="button" disabled={pending} onClick={onInbox}>현재 업무로 돌아가기</button> : null}
    ...
    {player ? <div id="career-market-player"><h3>{name(player.playerId)} 계약 협상</h3><button disabled={pending} onClick={() => { closingDetail.current = true; window.sessionStorage.removeItem(`career-player:market:${careerId}:${year}`); window.sessionStorage.removeItem(`career-trade-focus:${careerId}:${year}`); setSelected(null); setEditing(null); onCloseDetail?.(); }}>선수 계약 상세 닫기</button><p>{inclination(player.preference.inclination)} · 실제 제안과 조건에 따라 달라지는 게임 의향입니다.</p><p>새 계약 요구액 {money(player.askingSalary)} · {pricingLabel(player.pricing)} · 계약 종료와 공통 결정일을 반영한 시작 가능일 {player.availableStart ?? '현재 제안 불가'}</p>{player.eligibilityReason ? <p>{player.eligibilityReason}</p> : null}<p>{view.registrationPolicy}</p>
    ...
    <CareerTransferPanel focus={focus} view={view} roster={roster} selected={selected} disabled={disabled} onBegin={onBegin} onResult={async (next, message) => { if (message) setNotice(message); setView(old => newerMarket(old, next)); await onChanged(); }} />
  </section>;
}
