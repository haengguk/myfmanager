...
export function CareerCreateDialog({ teams, initial, pending, error, returnFocus, onClose, onCreate }: {
  ...
  const [saveName, setSaveName] = useState(initial?.saveName ?? 'GEN 장기 저장');
  const [managerName, setManagerName] = useState(initial?.managerName ?? '김 감독');
  const [teamCode, setTeamCode] = useState(initial?.managedTeamCode ?? teams[0]?.teamCode ?? '');
  ...
  return <div className="ca-modal-layer">
    <div ref={dialogRef} className="ca-dialog" role="dialog" aria-modal="true" aria-labelledby="ca-create-title" aria-describedby="ca-create-copy" onKeyDown={keyDown}>
      ...
      <p id="ca-create-copy">선택한 팀으로 전용 LCK Hybrid Season을 한 번 생성합니다. 날짜 진행이나 이적 기능은 아직 포함되지 않습니다.</p>
      <form onSubmit={submit} noValidate>
        <label><span>저장 이름</span><input ref={firstRef} value={saveName} onChange={(event) => setSaveName(event.target.value)} maxLength={80} autoComplete="off" disabled={pending} /></label>
        ...
        <label><span>관리 팀</span><select value={teamCode} onChange={(event) => setTeamCode(event.target.value)} disabled={pending || teams.length === 0}>{teams.map((team) => <option key={team.teamCode} value={team.teamCode}>{team.teamCode} · LCK 주전 {team.starterCount}명</option>)}</select></label>
