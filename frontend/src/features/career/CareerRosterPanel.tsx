...
const reasonName = (s: string | null) => s ? ({ AFFILIATION_UNCONFIRMED: '소속 미확인', V4_REGISTERED_ROLE_REVIEW_REQUIRED: '작성 포지션과 현재 등록 역할 검토 필요', NO_COMPETITIVE_TEAM: '실행 대상 경쟁 팀 없음', UNAFFILIATED_SIGNING_NOT_IMPLEMENTED: '무소속 · 게임 계약 시장에서 확인' }[s] ?? '현재 소속으로 출전할 수 없음') : null;
...
function PlayerDetails({ player }: { player: RosterPlayer }) {
  ...
  return <div className="ca-player-details"><h4>{player.nickname} · {player.position}</h4><p>{player.playerId}</p><p><strong>CA {currentAbility(player.gameplay.ratings)} · PA {potentialAbility(player) ?? '미입력'}</strong> · CA는 12개 능력치 동일 비중, PA는 평생 능력 천장의 작성값입니다.</p>
    <p>{player.provisional ? '추정 능력치 · 공식 측정값 아님 · 중립 숙련도 14와 저장된 작성값 사용' : '기존 게임 능력치'} · 자료 기준일 {show(d.snapshotAt ?? contract.checkedAt)}. 과거 조사 당시의 계약·나이이며 게임 날짜에 따라 자동 갱신되지 않습니다.</p>
    <dl><dt>실명</dt><dd>{show(personal.legalName)}</dd><dt>생년월일 / 조사 당시 나이</dt><dd>{show(personal.birthDate)} / {show(personal.ageAsOfSnapshot)}</dd><dt>국적 (표시 정보)</dt><dd>{show(personal.nationality)}</dd><dt>계약 종료 / 조사 당시 남은 일수</dt><dd>{show(contract.endDate)} / {show(contract.daysRemainingAsOfSnapshot)}</dd><dt>공식 경기 데뷔일</dt><dd>{show(career.debutDate)}</dd><dt>최초 확인 소속일 (데뷔일 아님)</dt><dd>{show((d.careerAuthoringDetail as Record<string, unknown> | undefined)?.firstKnownRosterRecordDate)}</dd><dt>조사 원문 배치 / 등록 역할</dt><dd>{show((d.roster as Record<string, unknown> | undefined)?.squadTier)} / {show(d.currentRegisteredRole)}</dd></dl>
