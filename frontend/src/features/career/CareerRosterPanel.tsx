...
export function CareerRosterPanel({ careerId, year, revision, historical, busy, onBegin, onChanged, onManageContract, focus, onInbox }: {
  onInbox?: () => void;
  ...
  useEffect(() => { if (focus?.panel === 'ROSTER') { setFilter('managed'); setSearch(focus.positions[0] ?? ''); setSelected(focus.playerId); } }, [focus]);
  ...
  useEffect(() => { setView(null); setMarket(null); setSelected(focus?.playerId ?? window.sessionStorage.getItem(`career-player:roster:${careerId}:${year}`)); setPending(false); return () => { mutation.current?.controller.abort(); mutation.current?.release(); mutation.current = null; }; }, [careerId, year]);
  useEffect(() => { if (view && focus?.panel === 'ROSTER') focusPlayTarget('#career-roster'); }, [view, focus]);
  ...
  return <section className="ca-calendar ca-roster" id="career-roster" aria-label="선수 명부" aria-busy={pending}>{focus?.panel === 'ROSTER' ? <p role="status">등록 대상 {focus.competition} · 필요한 포지션 {focus.positions.join(', ') || '선발·자격 확인'}</p> : null}{focus?.panel === 'ROSTER' ? <button type="button" disabled={pending} onClick={onInbox}>현재 업무로 돌아가기</button> : null}<header><div><span>선수 명부</span><strong>{view.seasonYear} 선수 명부 · 전체 {Object.keys(players).length}명</strong></div><span>명단 갱신 {view.revision}{historical || view.readOnly ? ' · 읽기 전용' : ''}</span></header>
