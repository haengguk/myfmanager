import { tradeProgress } from './careerPlayFlow';
...
export function CareerTransferPanel({ view, roster, selected, disabled, focus, onBegin, onResult }: { focus?: InboxLink | null; view: CareerMarket; roster: CareerRoster; selected: string | null; disabled: boolean; onBegin: () => (() => void) | null; onResult: (view: CareerMarket, notice?: string) => void | Promise<unknown> }) {
  ...
  const execute = async (action?: TradeCommand['action'], trade?: Negotiation) => {
    ...
    try {
      ...
      window.sessionStorage.removeItem(tradeOperationKey(view.careerId)); setOperation(null); setEditing(null); const confirmed = result.market.management?.trades.find(t => t.tradeId === (body.tradeId ?? result.receipt.referenceId));
      await onResult(result.market, confirmed ? tradeProgress(confirmed) : '거래 조건 접수 결과를 확인했습니다. 아래 원본 거래의 구단·선수 동의와 적용일을 확인하세요.');
    } catch (e) {
  ...
  return <div aria-label="유료 이적과 임대"><h3>유료 이적·임대 협상</h3><p>추정 가치는 저장된 능력치 기준 연봉과 남은 계약기간의 게임 계산입니다. 구단 요구액·실제 합의 금액과 다르며 선수의 동의도 필요합니다.</p>
    ...
    {contract ? <p>현재 원계약 연봉 {careerMoney(contract.terms.annualSalary, view.currency)} · {contract.terms.endDate}까지</p> : null}{quote ? <p>{nickname(quote.playerId)} · 새 계약 요구액 {careerMoney(quote.referenceSalary, view.currency)} · {pricingLabel(quote.pricing)} / 추정 이적가치 {careerMoney(quote.estimatedValue, view.currency)} · 판매 요구 참고 {careerMoney(quote.suggestedTransferFee, view.currency)}<br />{quote.unavailableReason ?? `적용 가능일 ${quote.earliestStart}부터 · 실제 조건은 적용일까지 재검사합니다.`}</p> : <p>시장 선수 목록에서 계약 중인 선수를 선택하세요.</p>}
    {(contract?.team && !quote?.unavailableReason || editing) && !view.readOnly ? <fieldset disabled={blocked}><legend>{editing ? `${nickname(editing.terms.playerId)} 역제안` : own ? '판매·임대 보내기 제안' : '구매·임대 받기 제안'}</legend>
      ...
      <p>임대는 원계약의 총급여를 유지하며 28~366일, 원계약 종료 이내입니다. 새 계약 요구액은 임대 급여 인상 조건이 아닙니다. 임대료는 구단 간 비용이고 급여 분담률은 지급 구단의 부담만 나눕니다. 선수 판단은 역할·기회·기간·이동 부담도 확인합니다. 원소속은 복귀 자리를 확보하고 임대 구단만 현재 선발을 관리합니다. 진행 중 Series와 기존 국제 등록은 보존합니다.</p>
      <button disabled={!buyer || !start || !end} onClick={() => void execute(editing ? 'COUNTER' : 'SUBMIT')}>{editing ? '역제안 제출' : '구단 간 제안 제출'}</button>
    </fieldset> : null}
    <h4>우리 구단 거래와 응답</h4>{view.management.trades.length ? view.management.trades.map(t => <article key={t.tradeId}><strong>{nickname(t.terms.playerId)} · {t.terms.kind === 'LOAN' ? '임대' : '이적'} · {names[t.status]}</strong><p role="status">{tradeProgress(t)}</p><p>{t.terms.seller} → {t.terms.buyer} · {careerMoney(t.terms.fee, view.currency)} · 적용 {t.terms.startDate} / 종료 {t.terms.endDate}<br />구단 응답 {t.responseDate} · 선수 공통 결정 {t.decisionDate} · {t.reason}</p>
