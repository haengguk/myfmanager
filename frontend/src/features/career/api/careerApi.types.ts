...
export interface CareerDomesticDecisionDto { competitionId: string; decisionId: string; status: 'RUNNING' | 'SEALED'; inputHash: string; policyVersion: string; detail: { pendingMatches?: readonly { matchId: string; first: string; second: string }[]; nextMatchId?: string | null; [key: string]: unknown } }
export interface CareerFinalRankingDto { status: 'SEALED'; sourceSeasonYear: number; sourceSeasonId: string; championTeamCode: string; runnerUpTeamCode: string; ranking: readonly { seed: number; teamCode: string; seriesWins: number; seriesLosses: number; gameWins: number; gameLosses: number }[]; stateHash: string; ruleVersion: string | null; policyVersion: string | null; resultEvidenceHash: string | null; worldsStatus: 'PENDING_IN_GAME_INTERNATIONAL_EVIDENCE'; requiredInternationalEvidence: readonly string[] }
export interface CareerInternationalDto {
  competitionId: string; ruleVersion: string; ruleResourceHash: string; policyVersion: string;
  selectionPolicy: string; rosterSnapshotIdentity: string;
  entries: readonly { team: string; region: string; regionalSeed: number; pool: number; phase: 'MAIN' | 'PLAY_IN'; qualification: string; playInSeed?: number }[];
  bracket: { bouts: readonly { id: string; stage: string; date: string; order: number; format: 'BO1' | 'BO3' | 'BO5'; first: string; second: string; selectionOwner: string; sidePolicy: string; group: string | null }[];
    placements: Readonly<Record<string, number>>; draws: readonly { scope: string; teams: readonly string[]; relaxation: string | null }[];
    regionalPerformance: readonly string[]; champion: string | null; complete: boolean };
  results: Readonly<Record<string, { winner: string; loser: string }>>;
}
export interface CareerCompetitionViewDto { internationalCompetitions?: readonly CareerInternationalDto[]; domesticRankingDecisions?: readonly CareerDomesticDecisionDto[]; finalRanking?: CareerFinalRankingDto | null; domesticRuleCompatibility?: string; schemaVersion: 'CAREER_COMPETITION_VIEW_V1'; calendarSeasonYear: number; ruleResourceHash: string; ruleVersion: 'lck-career-competition-rules-2026-v2' | 'lck-career-competition-rules-2026-v3'; gamePolicyVersion: 'CAREER_COMPETITION_GAME_POLICY_V2' | 'CAREER_COMPETITION_GAME_POLICY_V3'; projectionPolicy: 'SAME_LOCAL_MONTH_DAY_FROM_2026_REFERENCE_V1'; r3r4AllocationPolicy: 'LCK_R3_R4_TEN_MATCHDAYS_LINEAR_INCLUSIVE_WINDOW_V1'; lifecycleStatus: string; revision: number; stateHash: string | null; currentCompetition: CareerCompetitionSummaryDto | null; nextCompetition: CareerCompetitionSummaryDto | null; nextFixture: CareerCompetitionFixtureDto | null; qualificationOutputs: readonly { competitionId: string; outputId: string; teamCode: string }[]; groupStandings: readonly CareerCompetitionStandingDto[]; currentSeeds: readonly CareerCompetitionSeedDto[]; externalExecutionLimited: boolean; activePendingCommand: { clientCommandId: string; competitionId: string; matchId: string; commandStatus: 'PENDING' | 'RUNNING' } | null; allowedCommands: readonly string[] }
...
export interface CareerCompetitionCommandRequestDto { schemaVersion: typeof CAREER_SCHEMAS.competitionCommandRequest; expectedCompetitionRevision: number; clientCommandId: string; sourceYear?: number }
...
export interface CareerSeasonSummaryDto { year: number; ordinal: number; leagueId: string; seasonId: string; status: 'ACTIVE' | 'CLOSED'; rosterHash: string | null }
export interface CareerSeasonsDto { schemaVersion: 'CAREER_SEASONS_V1'; careerId: string; activeYear: number; calendarRevision: number; seasons: readonly CareerSeasonSummaryDto[]; blockers: readonly string[]; allowedCommands: readonly 'START_NEXT_SEASON'[] }
