...
const COMPETITION_IDS = ['LCK_CUP', 'LCK_REGULAR_R1_R2', 'LCK_ROAD_TO_MSI', 'LCK_REGULAR_R3_R4', 'LCK_PLAY_IN', 'LCK_PLAYOFFS', 'FIRST_STAND', 'MSI', 'EWC_LOL', 'WORLDS', 'ASIAN_GAMES_LOL_RELEASE'] as const;
...
function competitionSummary(value: unknown, path: string): RecordValue {
  const item = object(value, path); exactKeys(item, ['competitionId', 'stageId', 'ruleStatus', 'lifecycleStatus', 'blockingReason', 'revision', 'stateHash', 'completedFixtures', 'totalFixtures'], path); oneOf(item.competitionId, COMPETITION_IDS, `${path}.competitionId`); if (!/^[A-Z0-9_]+$/.test(text(item.stageId, `${path}.stageId`))) throw new CareerContractError(`${path}.stageId`, 'structured stage identity required'); oneOf(item.ruleStatus, ['RULE_SOURCE_COMPLETE', 'RULE_SOURCE_INCOMPLETE', 'PRODUCT_POLICY_REQUIRED'] as const, `${path}.ruleStatus`); text(item.lifecycleStatus, `${path}.lifecycleStatus`); nullableText(item.blockingReason, `${path}.blockingReason`); integer(item.revision, `${path}.revision`); identity(item.stateHash, SHA256, `${path}.stateHash`); const completed = integer(item.completedFixtures, `${path}.completedFixtures`); const total = integer(item.totalFixtures, `${path}.totalFixtures`); if (completed > total) throw new CareerContractError(path, 'fixture progress mismatch'); return item;
}

function competitionView(value: unknown, path: string, seasonYear: number): void {
  const item = object(value, path); exactKeys(item, ['schemaVersion', 'calendarSeasonYear', 'ruleResourceHash', 'ruleVersion', 'gamePolicyVersion', 'projectionPolicy', 'r3r4AllocationPolicy', 'lifecycleStatus', 'revision', 'stateHash', 'currentCompetition', 'nextCompetition', 'nextFixture', 'qualificationOutputs', 'externalExecutionLimited', 'activePendingCommand', 'allowedCommands'], path);
  if (item.schemaVersion !== 'CAREER_COMPETITION_VIEW_V1' || item.calendarSeasonYear !== seasonYear || item.ruleVersion !== 'lck-career-competition-rules-2026-v1' || item.gamePolicyVersion !== 'CAREER_COMPETITION_GAME_POLICY_V1' || item.projectionPolicy !== 'SAME_LOCAL_MONTH_DAY_FROM_2026_REFERENCE_V1' || item.r3r4AllocationPolicy !== 'LCK_R3_R4_TEN_MATCHDAYS_LINEAR_INCLUSIVE_WINDOW_V1') throw new CareerContractError(path, 'competition authority identity mismatch'); identity(item.ruleResourceHash, SHA256, `${path}.ruleResourceHash`); text(item.lifecycleStatus, `${path}.lifecycleStatus`); integer(item.revision, `${path}.revision`); identity(item.stateHash, SHA256, `${path}.stateHash`);
  ...
  if (item.nextFixture !== null) { const fixture = object(item.nextFixture, `${path}.nextFixture`); exactKeys(fixture, ['competitionId', 'matchId', 'fixtureId', 'seriesId', 'date', 'scheduleStatus', 'seriesFormat', 'hardFearless', 'firstTeamCode', 'secondTeamCode', 'executionMode', 'lifecycleStatus', 'managedTeamIncluded', 'rootSeed', 'seedAlgorithm'], `${path}.nextFixture`); oneOf(fixture.competitionId, COMPETITION_IDS, `${path}.nextFixture.competitionId`); text(fixture.matchId, `${path}.nextFixture.matchId`); identity(fixture.fixtureId, COMPETITION_FIXTURE_ID, `${path}.nextFixture.fixtureId`); identity(fixture.seriesId, SERIES_ID, `${path}.nextFixture.seriesId`); date(fixture.date, `${path}.nextFixture.date`); oneOf(fixture.scheduleStatus, ['OFFICIAL_PROJECTED_DATE', 'GAME_DERIVED_SCHEDULE_POLICY'] as const, `${path}.nextFixture.scheduleStatus`); oneOf(fixture.seriesFormat, ['BO3', 'BO5'] as const, `${path}.nextFixture.seriesFormat`); if (fixture.hardFearless !== true) throw new CareerContractError(`${path}.nextFixture.hardFearless`, 'hard Fearless required'); const first = fixture.firstTeamCode === null ? null : text(fixture.firstTeamCode, `${path}.nextFixture.firstTeamCode`); const second = fixture.secondTeamCode === null ? null : text(fixture.secondTeamCode, `${path}.nextFixture.secondTeamCode`); if (first !== null && !/^[A-Z0-9]{2,16}$/.test(first) || second !== null && !/^[A-Z0-9]{2,16}$/.test(second) || first !== null && first === second) throw new CareerContractError(`${path}.nextFixture`, 'team identity mismatch'); oneOf(fixture.executionMode, ['FULL_AUTO', 'PLAYER_CONTROLLED'] as const, `${path}.nextFixture.executionMode`); text(fixture.lifecycleStatus, `${path}.nextFixture.lifecycleStatus`); bool(fixture.managedTeamIncluded, `${path}.nextFixture.managedTeamIncluded`); if (!SIGNED_LONG.test(text(fixture.rootSeed, `${path}.nextFixture.rootSeed`)) || fixture.seedAlgorithm !== 'CAREER_COMPETITION_MATCH_SHA256_FIRST_8_BYTES_BIG_ENDIAN_SIGNED_LONG_V1') throw new CareerContractError(`${path}.nextFixture.rootSeed`, 'competition seed identity mismatch'); }
  if (!Array.isArray(item.qualificationOutputs) || item.qualificationOutputs.length > 16) throw new CareerContractError(`${path}.qualificationOutputs`, 'bounded array required'); const outputIds = new Set<string>(); item.qualificationOutputs.forEach((entry, index) => { const output = object(entry, `${path}.qualificationOutputs[${index}]`); exactKeys(output, ['competitionId', 'outputId', 'teamCode'], `${path}.qualificationOutputs[${index}]`); oneOf(output.competitionId, COMPETITION_IDS, `${path}.qualificationOutputs[${index}].competitionId`); const outputId = text(output.outputId, `${path}.qualificationOutputs[${index}].outputId`); if (!/^[A-Z0-9_]+$/.test(outputId) || !outputIds.add(outputId) || !/^[A-Z0-9]{2,16}$/.test(text(output.teamCode, `${path}.qualificationOutputs[${index}].teamCode`))) throw new CareerContractError(`${path}.qualificationOutputs[${index}]`, 'qualification identity mismatch'); });
  ...
  if (item.activePendingCommand !== null) { const pending = object(item.activePendingCommand, `${path}.activePendingCommand`); exactKeys(pending, ['clientCommandId', 'competitionId', 'matchId', 'commandStatus'], `${path}.activePendingCommand`); if (!UUID.test(text(pending.clientCommandId, `${path}.activePendingCommand.clientCommandId`)) || pending.commandStatus !== 'PENDING') throw new CareerContractError(`${path}.activePendingCommand`, 'pending competition command mismatch'); oneOf(pending.competitionId, COMPETITION_IDS, `${path}.activePendingCommand.competitionId`); text(pending.matchId, `${path}.activePendingCommand.matchId`); }
  if (!Array.isArray(item.allowedCommands) || item.allowedCommands.some((command) => typeof command !== 'string' || command.length === 0) || item.activePendingCommand !== null && item.allowedCommands.length !== 0) throw new CareerContractError(`${path}.allowedCommands`, 'authoritative competition commands required');
}

export function validateCareerCalendar(value: unknown): CareerCalendarViewDto {
  const root = object(value, '$'); exactKeys(root, ['schemaVersion', 'careerId', 'activeCalendarSeasonYear', 'currentDate', 'calendarRevision', 'lifecycleStatus', 'blockingReason', 'calendarStateHash', 'stateHashAlgorithm', 'provenance', 'projectionStatus', 'currentEvent', 'nextEvent', 'currentStage', 'nextStage', 'upcomingEvents', 'fixtureOverlay', 'upcomingFixtures', 'nextManagedFixture', 'allowedAdvanceModes', 'activePendingAdvance', 'competition', 'qualificationEdges', 'pendingOfficialFields', 'sourceDataNotes'], '$');
