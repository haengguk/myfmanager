...
export interface CareerDevelopment { schemaVersion: 'CAREER_DEVELOPMENT_VIEW_V1'; careerId: string; seasonYear: number; revision: number; currentDate: string; managedTeam: string; readOnly: boolean; teamPlan: TrainingSchedule | null; developmentTeamPlan?: TrainingSchedule | null; players: DevelopmentPlayer[]; recentChanges: DevelopmentGain[]; monthlySummaries: DevelopmentGain[]; legalChampions: Record<string, string[]>; policyVersion: string }
export interface TrainingCommand { squad?: 'FIRST_TEAM' | 'DEVELOPMENT'; schemaVersion: 'CAREER_TRAINING_COMMAND_V1'; sourceYear: number; expectedRevision: number; playerId: string | null; plan: TrainingPlan | null; clearOverride: boolean; clientCommandId: string }
...
export function validateTrainingCommand(value: unknown): TrainingCommand {
  const v = obj(value); check(Object.keys(v).filter(k => k !== 'squad').sort().join() === 'schemaVersion sourceYear expectedRevision playerId plan clearOverride clientCommandId'.split(' ').sort().join());
  check(v.squad === undefined || v.squad === 'FIRST_TEAM' || v.squad === 'DEVELOPMENT');
