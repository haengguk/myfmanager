...
import { currentNavigation, matchDestination } from './careerNavigation';
...
export function CareerDashboardPage({ searchValue, onResume, onOpenCompetitionSeries, onNotify }: {
  ...
  useEffect(() => {
    ...
    const target = document.querySelector(selectors[inboxFocus.panel] ?? '.ca-records'); if (target instanceof HTMLDetailsElement) target.open = true; target?.scrollIntoView({ behavior: 'smooth' });
