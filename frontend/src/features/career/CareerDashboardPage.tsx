...
import { calendarMatchLink, nextPlayLabel, focusPlayTarget } from './careerPlayFlow';
...
export function CareerDashboardPage({ searchValue, onResume, onOpenCompetitionSeries, onNotify, returnedSeries }: {
  ...
  const [continuous, setContinuous] = useState<{ career: string; active: boolean } | null>(null);
  ...
  useEffect(() => setInboxFocus(null), [selectedId]);
  ...
  const loadWorkspace = useCallback(async () => {
    ...
    try {
      ...
      if (nextList.careers.some((career) => !career.compatibility && !teamCodes.has(career.managedTeamCode))) throw new CareerApiFailure('CONTRACT', 'Career 목록에 현재 LCK reference에 없는 관리 팀이 있습니다.');
      setList(nextList); setTeams(teamResponse.teams); teamsRef.current = teamResponse.teams; catalogRef.current = catalog;
      const pointer = readCareerPointer(window.sessionStorage); const target = pointer ?? nextList.careers.find(career => career.compatibility?.status !== 'UNSUPPORTED')?.careerId ?? null;
      ...
      try {
        ...
        try { const calendarView = await getCareerCalendar(career.careerId, controller.signal); if (controller.signal.aborted || generation !== generationRef.current) return; reconcileCareerAdvanceOperation(window.sessionStorage, career.careerId, calendarView.activePendingAdvance); reconcileCareerCompetitionOperation(window.sessionStorage, career.careerId, calendarView.competition.revision, calendarView.competition.activePendingCommand, calendarView.activeCalendarSeasonYear); setCalendar(calendarView); setCalendarError(null); }
        catch (calendarCause) { if (!controller.signal.aborted && generation === generationRef.current) setCalendarError(loadFailure(calendarCause)); }
        finally { if (!controller.signal.aborted && generation === generationRef.current) setCalendarLoading(false); }
      } catch (cause) {
        if (controller.signal.aborted || generation !== generationRef.current) return;
        const failure = cause instanceof CareerApiFailure ? cause : new CareerApiFailure('NETWORK', loadFailure(cause)); const action = careerPointerRecoveryAction(failure);
  ...
  const create = useCallback(async (selection: CareerCreateSelection) => {
    ...
    } finally { if (isCurrent()) setCreatePending(false); if (createRequestRef.current === controller) createRequestRef.current = null; }
  ...
  const executeCompetition = useCallback(async () => {
    ...
    const body = {
      ...
      if (response.executionMode === 'FULL_AUTO' && ['PENDING', 'RUNNING'].includes(response.status)) {
        for (const delay of [400, 800, 1_200, 2_000, 3_000]) {
          await wait(delay, controller.signal); if (!isCurrent()) return;
          latest = await getCareerCalendar(detail.careerId, controller.signal); if (!isCurrent()) return; reconcileCareerCompetitionOperation(window.sessionStorage, detail.careerId, latest.competition.revision, latest.competition.activePendingCommand, latest.activeCalendarSeasonYear); setCalendar(latest);
  ...
  useEffect(() => {
    if (!detail || !calendar || continuousBusy || advancePending || historical || mutationGate.current!.busy) return;
  ...
  }, [advance, advancePending, calendar, detail, historical, mutationPending, continuousBusy]);
  ...
  const navigateCareer = (link: InboxLink) => {
    ...
    void currentNavigation(link, () => loadDetail(career.careerId), () => token === navigationGeneration.current && selectedIdRef.current === career.careerId, () => {
      ...
      if (link.panel === 'MATCH') {
        ...
        if (destination === 'SERIES') onOpenCompetitionSeries?.(link.seriesId!, career, '관리 경기');
        else if (destination === 'LEAGUE_PREPARATION') onResume(career);
      }
    });
  };
  useEffect(() => {
  ...
  }, [inboxFocus, calendarLoading, detailLoading]);
  return <main className="ca-workspace" aria-labelledby="ca-page-title">
    ...
    {initialLoading ? <section className="ca-loading" role="status" aria-live="polite"><span aria-hidden="true" /><strong>Career 저장 목록 확인 중</strong><p>브라우저 캐시가 아닌 서버의 최신 저장 상태를 읽고 있습니다.</p></section> : error && list.careers.length === 0 ? <section className={`ca-error${integrityError ? ' is-integrity' : ''}`} role="alert"><strong>{integrityError ? '저장 무결성 확인 필요' : 'Career를 불러오지 못했습니다'}</strong><p>{error}</p><button type="button" className="lm-secondary-button" onClick={() => { void loadWorkspace(); }}>다시 시도</button></section> : <div className="ca-layout">
      <aside className="ca-saves" aria-label="Career 저장 목록"><header><div><span>SAVED CAREERS</span><strong>{query ? `검색 결과 ${careers.length}` : `${list.currentCount}개 저장`}</strong></div><small>최근 운영 저장 순</small></header>{careers.length === 0 ? <div className="ca-empty"><strong>{list.currentCount === 0 ? '저장된 커리어가 없습니다' : '검색 결과가 없습니다'}</strong><p>{list.currentCount === 0 ? 'LCK 관리 팀과 감독 이름을 정해 첫 Career를 만드세요.' : '저장 이름, 감독 또는 팀 코드로 다시 검색하세요.'}</p>{list.currentCount === 0 && !full ? <button type="button" className="lm-primary-button" onClick={() => setDialogOpen(true)}>첫 커리어 만들기</button> : null}</div> : <ol>{careers.map((career) => <li key={career.careerId}><button type="button" aria-current={selectedId === career.careerId ? 'true' : undefined} onClick={() => { void loadDetail(career.careerId); }}><span className={`ca-save-state is-${career.resumeKind.toLowerCase()}`}>{RESUME_KIND_COPY[career.resumeKind]}</span><strong>{career.saveName}</strong><span>{career.compatibility?.managedTeamName ?? career.managedTeamCode} · {career.managerName}</span>{career.compatibility?.status === 'UNSUPPORTED' ? <small>{career.compatibility.message}</small> : null}<small><time dateTime={career.currentDate}>{career.currentDate}</time><time dateTime={career.updatedAt}>{dateTime(career.updatedAt)}</time></small></button></li>)}</ol>}</aside>
      <section className="ca-detail" aria-live="polite">{detailLoading ? <div className="ca-detail-loading" role="status"><span aria-hidden="true" /><strong>선택한 Career 확인 중</strong><p>저장된 구단·시즌과 진행 상태를 확인합니다.</p></div> : error ? <div className={`ca-error${integrityError ? ' is-integrity' : ''}`} role="alert"><strong>{integrityError ? '복구 불가 상태 확인 필요' : '상세를 불러오지 못했습니다'}</strong><p>{error}</p>{selectedId ? <button type="button" className="lm-secondary-button" onClick={() => { void loadDetail(selectedId); }}>다시 시도</button> : null}</div> : detail ? <><header><div><span>ACTIVE CAREER</span><h2 ref={detailTitleRef} tabIndex={-1}>{detail.saveName}</h2><p>{detail.managedTeamCode} 구단을 맡은 {detail.managerName} 감독의 저장입니다.</p></div><span className="ca-active-mark">ACTIVE</span></header><div className="ca-facts"><dl><div><dt>관리 팀</dt><dd>{detail.managedTeamCode}</dd></div><div><dt>감독</dt><dd>{detail.managerName}</dd></div><div><dt>게임 날짜</dt><dd>{detail.currentDate}</dd></div><div><dt>Career 상태</dt><dd>{detail.lifecycleStatus}</dd></div></dl><section><span>선택 시즌</span><h3>{historical ? historyYear : calendar?.activeCalendarSeasonYear ?? '확인 중'}</h3><p>{historical ? '과거 기록 조회 · 운영 명령 없음' : '현재 운영 시즌'}</p><details><summary>연결된 정규리그·저장 상세</summary><p>R1–R2 Round {detail.resume.currentRound} / 18 · {detail.resume.seasonLifecycleStatus}</p><dl><div><dt>순위표 버전</dt><dd>{detail.resume.standingsRevision}</dd></div><div><dt>진행 버전</dt><dd>{detail.resume.lifecycleRevision}</dd></div><div><dt>운영 저장 시각</dt><dd>{dateTime(detail.updatedAt)}</dd></div></dl></details></section></div>{detail.compatibility?.sourceChanged ? <p role="status">기본 선수 데이터가 갱신되었습니다. 이 Career는 저장된 선수·계약·성장 상태로 계속 진행하며, 최신 기본 데이터는 새 Career에 적용됩니다.</p> : null}{returnedSeries?.careerId === detail.careerId ? <CareerReturnResult key={`${detail.careerId}:${returnedSeries.seriesId}`} careerId={detail.careerId} seriesId={returnedSeries.seriesId} onNavigate={navigateCareer} /> : null}{inboxFocus ? <p role="status">선택한 원본 업무·기록을 확인하고 있습니다. 조치 후 최신 업무에서 다음 행동을 선택하세요. <button type="button" onClick={() => { setInboxFocus(null); focusPlayTarget('[aria-label="Career 소식함"]'); }}>현재 업무로 돌아가기</button></p> : null}{calendar ? <CareerInboxPanel key={`inbox:${detail.careerId}`} careerId={detail.careerId} year={calendar.activeCalendarSeasonYear} revision={calendar.calendarRevision + operatingRevision} running={continuousBusy} busy={mutationPending || continuousBusy} onNavigate={navigateCareer} /> : null}{calendar ? <CareerScoutingPanel key={`scouting:${detail.careerId}`} careerId={detail.careerId} year={calendar.activeCalendarSeasonYear} revision={calendar.calendarRevision + operatingRevision} busy={mutationPending || continuousBusy} onNavigate={navigateCareer} showOpponent={opponentRequest} /> : null}<CareerSeasonsPanel selectedYear={historical ? historyYear : null} key={detail.careerId} careerId={detail.careerId} revision={calendar?.calendarRevision ?? 0} busy={mutationPending || continuousBusy} onBegin={() => { if (continuousBusy) return null; const release = mutationGate.current!.acquire(); if (!release) return null; invalidateScreenRequests(); return release; }} onChanged={async () => { setOperatingRevision(v => v + 1); await loadDetail(detail.careerId); }} onHistory={(past, year) => { ++navigationGeneration.current; setInboxFocus(null); setHistorical(past); setHistoryYear(past ? year ?? null : null); }} onReplay={(series, matchup) => onOpenCompetitionSeries?.(series, detail, matchup)} />{!historical && calendar ? <CareerContinuousPanel key={`continuous:${detail.careerId}`} careerId={detail.careerId} currentDate={calendar.currentDate} seasonYear={calendar.activeCalendarSeasonYear} busy={mutationPending} onBusy={(career, active) => setContinuous({ career, active })} onBegin={() => mutationGate.current!.acquire()} onAction={(action) => { if (action === 'MATCH') { const link = calendarMatchLink(calendar); if (link) navigateCareer(link); else focusPlayTarget('[aria-label="Career 캘린더"]'); } else focusPlayTarget(action === 'SEASONS' ? '[aria-label="시즌 전환과 기록"]' : '[aria-label="Career 소식함"]'); }} appliedRun={appliedContinuous.current.get(detail.careerId) ?? null} onStopped={async stamp => { setOperatingRevision(v => v + 1); return await loadDetail(detail.careerId, false, stamp) === true; }} /> : null}{!historical ? <CareerCalendarPanel onClosePreparation={() => setInboxFocus(null)} onAnalyzeOpponent={() => { setOpponentRequest(v => v + 1); document.querySelector('[aria-label="Career 스카우팅"]')?.scrollIntoView({ behavior: 'smooth' }); }} focusFixture={inboxFocus?.panel === 'MATCH' ? inboxFocus.sourceId : null} calendar={calendar} loading={calendarLoading} pending={advancePending || mutationPending || continuousBusy} competitionPending={competitionPending || mutationPending || continuousBusy} error={calendarError} onAdvance={(mode) => { void advance(mode); }} onCompetitionAction={() => { void executeCompetition(); }} onRefresh={() => { void loadDetail(detail.careerId); }} /> : null}{calendar ? <CareerOverseasPanel careerId={detail.careerId} year={historical && historyYear ? historyYear : calendar.activeCalendarSeasonYear} revision={calendar.calendarRevision + calendar.competition.revision + operatingRevision} onReplay={(series, matchup) => onOpenCompetitionSeries?.(series, detail, matchup)} /> : null}{calendar ? <CareerClPanel key={`cl:${detail.careerId}`} careerId={detail.careerId} year={historical && historyYear ? historyYear : calendar.activeCalendarSeasonYear} revision={calendar.calendarRevision + operatingRevision} historical={historical} busy={mutationPending || continuousBusy} nextMatch={calendar.competition?.nextFixture?.competitionId === 'LCK_CL' ? calendar.competition.nextFixture.matchId : null} onBegin={() => historical || continuousBusy ? null : mutationGate.current!.acquire()} onChanged={async () => { setOperatingRevision(v => v + 1); await loadDetail(detail.careerId); }} onExecute={() => { void executeCompetition(); }} onReplay={(series, matchup) => onOpenCompetitionSeries?.(series, detail, matchup)} /> : null}{calendar ? <CareerRosterPanel focus={inboxFocus} onManageContract={id => { setMarketPlayer(id); document.getElementById("career-contract-market")?.scrollIntoView({ behavior: "smooth" }); }} key={`roster:${detail.careerId}`} careerId={detail.careerId} year={historical && historyYear ? historyYear : calendar.activeCalendarSeasonYear} revision={calendar.calendarRevision + operatingRevision} historical={historical} busy={mutationPending || continuousBusy} onBegin={() => historical || continuousBusy ? null : mutationGate.current!.acquire()} onChanged={async () => { setOperatingRevision(v => v + 1); await loadDetail(detail.careerId); }} /> : null}{calendar ? <CareerLifecyclePanel focusPlayer={inboxFocus?.panel === 'LIFECYCLE' ? inboxFocus.playerId : null} key={`lifecycle:${detail.careerId}`} careerId={detail.careerId} year={historical && historyYear ? historyYear : calendar.activeCalendarSeasonYear} revision={calendar.calendarRevision + operatingRevision} historical={historical} busy={mutationPending || continuousBusy} onMarket={id => { setMarketPlayer(id); document.getElementById("career-contract-market")?.scrollIntoView({ behavior: "smooth" }); }} /> : null}{calendar ? <CareerTrainingPanel focusPlayer={inboxFocus?.panel === 'TRAINING' ? inboxFocus.playerId : null} key={`training:${detail.careerId}`} careerId={detail.careerId} year={historical && historyYear ? historyYear : calendar.activeCalendarSeasonYear} revision={calendar.calendarRevision + operatingRevision} historical={historical} busy={mutationPending || continuousBusy} onBegin={() => historical || continuousBusy ? null : mutationGate.current!.acquire()} onChanged={async () => { setOperatingRevision(v => v + 1); await loadDetail(detail.careerId); }} /> : null}{calendar ? <CareerMarketPanel focus={inboxFocus} key={`market:${detail.careerId}`} careerId={detail.careerId} year={historical && historyYear ? historyYear : calendar.activeCalendarSeasonYear} revision={calendar.calendarRevision + operatingRevision} historical={historical} busy={mutationPending || continuousBusy} focusPlayer={marketPlayer} onBegin={() => historical || continuousBusy ? null : mutationGate.current!.acquire()} onChanged={async () => { setOperatingRevision(v => v + 1); await loadDetail(detail.careerId); }} /> : null}<CareerRecordsPanel focus={inboxFocus && ['RECORDS', 'AWARDS'].includes(inboxFocus.panel) ? inboxFocus : null} key={`records:${detail.careerId}`} careerId={detail.careerId} year={calendar?.activeCalendarSeasonYear ?? Number(detail.currentDate.slice(0, 4))} revision={(calendar?.calendarRevision ?? 0) + operatingRevision} busy={continuousBusy || mutationPending} /><section className="ca-identities"><span>SERVER-OWNED IDENTITY</span><dl><div><dt>Career</dt><dd><code>{detail.careerId}</code></dd></div><div><dt>League</dt><dd><code>{detail.leagueId}</code></dd></div><div><dt>Season</dt><dd><code>{detail.seasonId}</code></dd></div></dl></section></> : <div className="ca-empty ca-empty--detail"><strong>저장을 선택하세요</strong><p>선택한 저장의 최신 구단과 진행 상태를 표시합니다.</p></div>}</section>
      <aside className="ca-resume" aria-label="이어하기 문맥">{detail ? <><span>현재 상황과 다음 행동</span><strong>{detail.managedTeamCode} · {calendar?.currentDate ?? detail.currentDate}</strong><p>{historical ? `${historyYear}년 과거 기록을 보고 있습니다.` : `${calendar?.activeCalendarSeasonYear ?? ''} 시즌 운영 중`}</p><button type="button" className="lm-primary-button ca-resume__action" disabled={mutationPending || detailLoading} onClick={() => { if (historical) void loadDetail(detail.careerId); else focusPlayTarget(continuousBusy ? '[aria-label="Career 연속 진행"]' : '[aria-label="Career 캘린더"]'); }}>{nextPlayLabel(calendar, historical, continuousBusy)}</button><button type="button" className="lm-secondary-button" onClick={() => focusPlayTarget('[aria-label="Career 소식함"]')}>최신 현재 업무 보기</button><p>화면 이동은 경기 시작·계약 수락·날짜 진행을 실행하지 않습니다. 연결된 화면의 버튼으로 직접 선택하세요.</p><details><summary>연결된 정규리그 보기</summary><p>R1–R2의 보조 화면입니다. 현재 대회와 날짜 진행은 캘린더를 기준으로 확인하세요.</p><button type="button" className="lm-secondary-button" disabled={historical || mutationPending || continuousBusy} onClick={() => { if (!mutationGate.current!.busy) onResume(detail); }}>{CAREER_RESUME_COPY[detail.resume.kind].label}</button></details><button type="button" className="lm-text-button" disabled={mutationPending} onClick={() => { void loadDetail(detail.careerId); }}>저장 상태 새로고침</button></> : <><span>다음 행동</span><strong>저장 선택 대기</strong><p>목록에서 이어갈 Career를 선택하세요.</p></>}</aside>
    </div>}
