...
export const eventPresentation: Record<MatchEventType, { label: string; tone: string }> = {
  ...
  VOID_GRUB: { label: '공허 유충', tone: 'objective' },
  RIFT_HERALD: { label: '전령 확보', tone: 'objective' },
  HERALD_SUMMON: { label: '전령 소환', tone: 'objective' },
  HERALD_CHARGE: { label: '전령 돌진', tone: 'objective' },
  HERALD_EXPIRED: { label: '전령 소멸', tone: 'phase' },
  BASE_RETURN: { label: '수비 복귀', tone: 'accent' },
  BASE_DEFENSE: { label: '본진 수비', tone: 'teamfight' },
