...
@RestControllerAdvice(assignableTypes = {CareerApiV1Controller.class, CareerRosterApiV1Controller.class})
public final class CareerApiV1ExceptionHandler {
    ...
    public ResponseEntity<CareerApiV1Dtos.ErrorResponse> career(CareerException error) {
        HttpStatus status = switch (error.type()) {
            ...
            case COMMAND_CONFLICT, CAPACITY_REACHED,
                    ...
                    CALENDAR_MIGRATION_REQUIRED, COMPETITION_STALE_REVISION,
                    COMPETITION_COMMAND_CONFLICT -> HttpStatus.CONFLICT;
            case CALENDAR_BACKGROUND_UNAVAILABLE,
                    COMPETITION_BACKGROUND_UNAVAILABLE -> HttpStatus.SERVICE_UNAVAILABLE;
    ...
    private static String code(CareerException.Type type) {
        return switch (type) {
            ...
            case COMPETITION_STALE_REVISION ->
                    "CAREER_COMPETITION_STALE_REVISION";
            case COMPETITION_COMMAND_CONFLICT ->
                    "CAREER_COMPETITION_COMMAND_CONFLICT";
            case COMPETITION_BACKGROUND_UNAVAILABLE ->
                    "CAREER_COMPETITION_BACKGROUND_UNAVAILABLE";
        };
    }
