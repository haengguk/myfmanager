...
public final class LeagueSeasonApplicationService {
    ...
    public SeasonView cancel(String seasonId, long expectedLifecycleRevision) {
        return cancel(seasonId, expectedLifecycleRevision, () -> { });
    }

    SeasonView cancel(
            String seasonId,
            long expectedLifecycleRevision,
            Runnable afterSeasonTransition
    ) {
        Objects.requireNonNull(afterSeasonTransition, "afterSeasonTransition");
        return store.transactions().execute(ignored -> {
            store.lockGlobalFixtureLeases();
            lockSeason(seasonId);
            transition(seasonId, expectedLifecycleRevision,
                    List.of(LeaguePersistenceState.SeasonStatus.READY,
                            LeaguePersistenceState.SeasonStatus.RUNNING,
                            LeaguePersistenceState.SeasonStatus.PAUSED,
                            LeaguePersistenceState.SeasonStatus.WAITING_FOR_PLAYER),
                    LeaguePersistenceState.SeasonStatus.CANCELLED);
            afterSeasonTransition.run();
            store.jdbc().update("""
    ...
    public SeasonView view(String seasonId) {
        List<SeasonView> rows = store.jdbc().query("""
                SELECT season_id, lifecycle_status, lifecycle_revision, revision,
                       ...
                       frozen_snapshot_hash, product_decision_hash, updated_at
                ...
                result.getObject(10, java.time.OffsetDateTime.class)), seasonId);
    ...
    void lockSeason(String seasonId) {
        List<String> rows = store.jdbc().query("""
                SELECT season_id FROM league_season
                WHERE season_id = ? FOR UPDATE
