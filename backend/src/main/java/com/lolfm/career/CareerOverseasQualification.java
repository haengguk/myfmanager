...
final class CareerOverseasQualification {
    ...
    static final class Waiting extends RuntimeException {
        final String code,requiredEvent,team;
        Waiting(String code,String requiredEvent,String team){super(code+":"+(team==null?requiredEvent:team));this.code=code;this.requiredEvent=requiredEvent;this.team=team;}
    }
    ...
    static CareerInternationalParticipants.Selection select(CareerCompetitionRelationalStore store,String career,int year,String competition){
        ...
        for(Event event:required(competition)){
            var state=CareerOverseasStore.load(store.jdbc,career,year,event);if(state==null||!state.plan().complete())throw new Waiting("OVERSEAS_RESULT_REQUIRED",event.name(),null);
            ...
            for(int i=0;i<order.teams().size();i++){String team=order.teams().get(i);CompetitionRosterSnapshot.Roster roster;
                try{roster=CareerOverseasRoster.roster(market,team);}catch(CareerException missing){if(i<required||team.equals(champion)&&eligible.contains(team))throw new Waiting("ROSTER_REPAIR_REQUIRED",null,team);else continue;}
