...
public final class CareerClStore {
    ...
    public void recover(){for(String id:db.query("SELECT career_id FROM career_player_directory WHERE directory_version=? ORDER BY career_id",(r,n)->r.getString(1),com.lolfm.player.ExpandedPlayerCatalog.VERSION))tx.executeWithoutResult(s->{lockCareer(db,id);if(activation(db,id)==null)db.update("UPDATE career_player_directory SET cl_activation_year=?,cl_introduced_on=? WHERE career_id=?",activeYear(db,id)+1,CareerMarketStore.date(db,id),id);else if(active(db,id,activeYear(db,id)))load(db,id,activeYear(db,id));});}
