...
public final class CareerAwardsStore {
    public record Candidate(String playerId,String name,String team,com.lolfm.domain.Position position,int sets,BigDecimal participation,
                            ...
                            @com.fasterxml.jackson.annotation.JsonInclude(com.fasterxml.jackson.annotation.JsonInclude.Include.NON_NULL) SampleEvidence sampleEvidence) {
        public Candidate(String playerId,String name,String team,com.lolfm.domain.Position position,int sets,BigDecimal participation,String eligibility,String reason,BigDecimal score,BigDecimal combat,Period period,String tieKey){this(playerId,name,team,position,sets,participation,eligibility,reason,score,combat,period,tieKey,null);}
    }
    public record SampleEvidence(int observedSeries,BigDecimal eligibleOpportunities,BigDecimal participationRate) {}
    ...
    public record Award(String instanceId,String definitionId,String name,String scope,String occurrence,int gameYear,int referenceYear,
                        ...
                        String tiePolicy,String publicationStatus,LocalDate publishedOn,List<String> stateHistory, @com.fasterxml.jackson.annotation.JsonInclude(com.fasterxml.jackson.annotation.JsonInclude.Include.NON_NULL) String evaluationVersion) {}
    ...
    static boolean complete(List<CareerRecordsStore.Series> series){return series.stream().flatMap(s->s.games().stream()).flatMap(g->g.players().stream()).map(p->p.evaluation().version()).distinct().count()==1&&!series.isEmpty()&&series.stream().allMatch(s->s.coverage().equals("COMPLETE")&&s.games().stream().allMatch(g->g.players().size()==10&&g.players().stream().allMatch(p->p.evaluation().status().equals("COMPLETE")&&Set.of(CareerPerformancePolicy.VERSION,CareerPerformanceV2.VERSION).contains(p.evaluation().version()))));}
    ...
    static List<Candidate> seriesCandidates(JdbcTemplate db,CareerRecordsStore.Series s,String definition,String occurrence,boolean winnerOnly) {
        ...
        for(var list:grouped.values()) {
            ...
            boolean v2=CareerPerformanceV2.VERSION.equals(last.evaluation().version());
            var score=v2?fixed(div(list.stream().map(p->p.evaluation().rating()).reduce(BigDecimal.ZERO,BigDecimal::add),decimal(list.size()))):series(list.stream().map(p->p.evaluation().rating()).toList(),s.games().size());
            ...
            result.add(new Candidate(last.playerId(),last.name(),last.team(),last.statistics().position(),list.size(),div(decimal(list.size()),decimal(s.games().size())),v2&&!CareerPerformanceV2.seriesEligible(list.size(),s.games().size())?"FALSE":"TRUE",v2?"실제 "+list.size()+"/"+s.games().size()+"세트 · 출전 기준(단일 세트 경기 또는 2세트·절반 이상) "+(!CareerPerformanceV2.seriesEligible(list.size(),s.games().size())?"미충족":"충족")+(winnerOnly?" · 우승 팀 결승 평균":" · 실제 평균 · 승리 팀 +3"):winnerOnly?"우승 팀 결승 실제 출전자 · 게임 정책":"실제 출전자 · 승리 팀 선정 점수 +3",score,combat,null,CareerAwardPolicy.tie(seed,instance,last.playerId())));
        }return result;
    }
    static boolean v2Period(Candidate c){return c.period()!=null&&CareerPerformanceV2.VERSION.equals(c.period().version());}
    static Comparator<Candidate> order(){return Comparator.comparing(Candidate::score,Comparator.reverseOrder()).thenComparing(Candidate::combat,Comparator.reverseOrder()).thenComparing(c->v2Period(c)?BigDecimal.ZERO:c.participation(),Comparator.reverseOrder()).thenComparing(Candidate::tieKey).thenComparing(Candidate::playerId);}
    ...
    static String reason(List<Candidate> ordered,int index) {
        ...
        if(a.combat().compareTo(b.combat())!=0)return v2Period(a)?"동점 · 같은 범위 관측 평균 우위":"동점 · 같은 범위 전투/보정 평균 우위";
        if(!v2Period(a)&&a.participation().compareTo(b.participation())!=0)return "동점 · 실제 참여 우위";
    ...
    static void save(JdbcTemplate db,String career,int year,String definition,String name,String scope,String occurrence,long revision,LocalDate date,List<CareerRecordsStore.Series> input,List<String> aliases,boolean badge,List<Candidate> candidates,boolean complete,CareerAwardPolicy.Definition policy) {
        ...
        String adopted=CareerEvaluationVersion.get(db,career,year);
        complete=complete&&input.stream().flatMap(s->s.games().stream()).flatMap(g->g.players().stream()).allMatch(p->adopted.equals(p.evaluation().version()));
        ...
        var inputs=input.stream().map(s->s.recordId()+":"+s.receiptHash()).distinct().sorted().toList();var rights=new ArrayList<Entitlement>();
        ...
        var award=new Award(id,definition,name,scope,occurrence,year,2026,status,complete?unknown?"후보의 과거 등록·신인 후보 이력이 확인되지 않아 시상을 확정하지 않았습니다.":"실제 성적과 게임 선정 정책으로 확정했습니다.":"평가 범위에 수집되지 않은 경기 성적이 있습니다.",CareerAwardPolicy.VERSION,CareerAwardPolicy.SOURCE_HASH,revision,date,inputs,hash(write(inputs)),aliases,badge,ordered,slots,rights,ordered.stream().anyMatch(CareerAwardsStore::v2Period)?"선정 점수 → 같은 범위 관측 평균 → 저장 seed와 시상·선수 identity의 결정적 해시 (표본 수·신뢰 보정은 순위 제외)":"선정 점수 → 같은 범위 전투 점수/보정 평균 → 실제 참여 → 저장 seed와 시상·선수 identity의 결정적 해시",status.equals("FINALIZED")?"PUBLISHED":"WITHHELD",status.equals("FINALIZED")?CareerMarketStore.date(db,career):null,List.of("COLLECTING","INPUT_SEALED",status),CareerPerformanceV2.VERSION.equals(CareerEvaluationVersion.get(db,career,year))?CareerPerformanceV2.VERSION:null);
