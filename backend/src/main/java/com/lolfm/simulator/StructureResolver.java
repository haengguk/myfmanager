...
public class StructureResolver {
    ...
    public Optional<StructureOutcome> destroyNextStructure(GameState state, TeamSide attackingSide, Lane lane, PushReason reason) {
        ...
        if(tower.isPresent()) return destroyTower(state,attackingSide,defendingSide,lane,tower.get(),reason,source(reason),null);
        ...
        if(state.getMapState().areNexusTurretsVulnerable(defendingSide) && base.destroyOneNexusTurret()) return Optional.of(new StructureOutcome(attackingSide,defendingSide,StructureKind.NEXUS_TURRET,null,null,state.getCurrentTimeSeconds(),reason,false));
    ...
    public Optional<StructureOutcome> destroyOuterFromLanePressure(GameState state,TeamSide attackingSide,Lane lane,OuterTurretSiegeData siege){
        if(state.isFinished())return Optional.empty();
        TeamSide defendingSide=attackingSide.opposite();
        ...
        return destroyTower(state,attackingSide,defendingSide,lane,TowerTier.OUTER,PushReason.MACRO_PLAY,StructureActionSource.LANE_PRESSURE,siege);
    }

    private Optional<StructureOutcome> destroyTower(GameState state,TeamSide attacking,TeamSide defending,Lane lane,TowerTier tier,PushReason reason,StructureActionSource source,OuterTurretSiegeData siege){
        ...
        return Optional.of(new StructureOutcome(attacking,defending,StructureKind.TOWER,lane,tier,state.getCurrentTimeSeconds(),reason,false,source,siege));
    }
    ...
    public MatchEvent createStructureEvent(GameState state, StructureOutcome outcome) {
        ...
        MatchEvent event=new MatchEvent(outcome.occurredAtSeconds(), MatchEventType.TOWER, message(team,outcome,state), null,null,List.of());
    ...
    private StructureActionSource source(PushReason reason){return switch(reason){case POST_FIGHT->StructureActionSource.POST_FIGHT;case BARON_PRESSURE->StructureActionSource.BARON_PRESSURE;case MACRO_PLAY->StructureActionSource.MACRO_PLAY;};}
    ...
    private String message(String team, StructureOutcome o, GameState state) {
        if(o.source()==StructureActionSource.LANE_PRESSURE)return team+"가 라인 압박으로 "+laneName(o.lane())+" 외곽 포탑을 파괴합니다.";
    ...
    private String laneName(Lane lane){return switch(lane){case TOP->"탑";case MID->"미드";case BOT->"바텀";};}
}
