...
public final class LateGameState{
 private final boolean enabled;private int started=-1,next=-1,sequence,lastProcessed=-1;private LateGameTransitionReason reason;private LateGameDecisionData latest;private final EnumMap<TeamSide,LateGameTeamPlanState> plans=new EnumMap<>(TeamSide.class);private final EnumMap<TeamSide,Integer>nextBaseDefenseCombatAt=new EnumMap<>(TeamSide.class);private final LateGameExecutionStats stats=new LateGameExecutionStats();
 public LateGameState(boolean enabled){this.enabled=enabled;for(TeamSide side:TeamSide.values()){plans.put(side,new LateGameTeamPlanState());nextBaseDefenseCombatAt.put(side,0);}if(!enabled)plans.values().forEach(LateGameTeamPlanState::disable);}
 ...
 public boolean canAttemptBaseDefenseCombat(TeamSide attackingSide,int time){return time>=nextBaseDefenseCombatAt.get(attackingSide);}
 public void recordBaseDefenseCombat(TeamSide attackingSide,int time){nextBaseDefenseCombatAt.put(attackingSide,time+LateGameRuleConfig.BASE_DEFENSE_COMBAT_COOLDOWN_SECONDS);}
