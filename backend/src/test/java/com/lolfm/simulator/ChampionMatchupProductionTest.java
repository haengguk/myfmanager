...
class ChampionMatchupProductionTest {
    ...
    @Test void productionCatalogHasExactCoverage() { assertThat(profiles.profiles()).hasSize(30); for(Position p:Position.values())assertThat(profiles.profiles().keySet()).filteredOn(k->k.position()==p).hasSize(6); }
