...
public final class PairInteractionDynamicAudit {
    ...
    public List<ChampionMatchupIndependentRow> generate(ChampionCatalog champions) {
        ...
        if (rows.size() != 32_400) throw new IllegalStateException(
                "Expected 32,400 interaction dynamic rows");
    ...
    private ChampionMatchupIndependentRow row(
        ...
        var generated = formula.evaluate(
                profiles.find(new ChampionRoleKey(source, position)).orElseThrow(),
                profiles.find(new ChampionRoleKey(opponent, position)).orElseThrow(),
                context);
        boolean sourceFavored = generated.finalEdge() >= 0;
        ...
        double matchup = Math.abs(generated.finalEdge());
        double afterMatchup = afterChampion + matchup;
        ...
        return new ChampionMatchupIndependentRow(
                ...
                afterChampion, matchup, 0, matchup, afterMatchup,
