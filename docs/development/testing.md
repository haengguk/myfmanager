...
[상세 관측·검증·적용 한계](career-long-term-operations-balance-and-navigation-fix-v1.md)와 [소형 CSV](career-long-term-operations-balance-and-navigation-fix-v1.csv)를 따른다.

장기 모델은 기본 test에서 제외한다. backend에서 `gradlew.bat -I scripts/career-long-term.init.gradle runCareerLongTermProbe -PprobeMode=long-term-smoke --console=plain --no-daemon`로 비용/준비를 확인한다. `long-term`은2seed×3시즌, `long-term-first`는첫seed×3시즌이며 DB는별도 JVM의 메모리 H2다. 같은 Gradle 출력 경로에서 진단/컴파일/test를 동시에 실행하지 않는다. G1만 확인할 때는 frontend `npm.cmd run career:verify`와 `npm.cmd run build`를 사용하며 backend 전체를 자동 실행하지 않는다.
