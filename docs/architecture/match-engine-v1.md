...
입력 생성의 완전 자동 경로는 `MatchEngineV1InputFactory`가 authoritative `FinalDraftResult`를 변환하는 것이다. Player Draft의 raw result 경로는 `PlayerControlledDraftMatchInputBoundary.validateAndCreateInput` 하나다. Public mixed boundary는 BLUE/RED team code, seed와 completed result만 받고 caller가 만든 `Team`이나 display name은 받지 않는다. `LckTeamAssembler`가 code별 authoritative five-position stable-`PlayerId` roster를 경계 내부에서 조립하며, unknown/same team을 거부한다. 이 roster로 team/seed/Game 1, 20턴 manual selectable-set legality와 authoritative AI trace, state hash, final role/player assignment를 처음부터 재구성하고 completed result의 Draft Meta version 및 required/actual legal-role hash를 current active `DraftResourceSet.meta()`와 exact 비교한다. 모두 통과해 private-constructor validated token이 만들어진 뒤에만 factory의 package-private projection을 호출한다. Public raw mixed factory와 caller 호출 순서 의존 preflight는 없다. V1 실행 중 재-Draft하지 않는다. 누락·중복 player/position/champion, roster/assignment/Draft hash 불일치, stale/forged Meta identity, illegal champion-role, candidate policy 요청은 Match Engine input, simulator와 seeded gameplay `Random` 생성 전에 거부한다.
