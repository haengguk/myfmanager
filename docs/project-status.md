...
AI 공백 보완에서 재정 검사 전 후보24명 절단을 제거한 `CAREER_SQUAD_PLANNING_V3`를 적용했다. 기존 V1/V2 판단·대기 기간·계약을 보존하며 다음 주간 검토부터 새 탐색을 사용한다. 공통 승인8회와 주간 제안/거래 한도, 사용자 결정, 성장·PA·재정·생애주기 계수는 유지했다.
고정2seed×3시즌의 전체 모집단 운영 모델과 동일 첫 seed 교정 비교를 완료했다. 대표 세 번째 시즌의 5명 확보 팀-일 비율은58.81→66.01%, 종료 AI 팀은15→21/57, 연중 매일 부족 팀은21→11이었다. 실제 경기 없는 모델이며 남은 재정/명부 문제를 균형 해결로 해석하지 않는다. 현금 원장·거래료 보존은 통과했다. 해외 활성 신규59계좌/58참가와 기존 이월 fixture56계좌를 구분한다.
집중3회/101건 시도 중 수정 전25명 재현 실패2건을 교정했고 최종 집중60건은 모두 통과했다. 최종 Career verifier137 PASS/build(Vite4.61초), 실제 선택 callback 검사와 같은 저장 복귀를 확인했다. 장기 실행57.69분(별도 초기 컴파일 포함58.53분), 추가 브라우저0회다. 계획 전체1회는286 suites/2,273건 중2,271통과·실패/오류0·기존 skip2(wall1,221.207초)로 완료했다. 원본 로그/XML을 보존했고 추가 전체/후속 집중0회다. 기존 실제 Auto BO3 1개·3세트의 완료/replay/Player 정지를 전체 안에서 확인했다.
[상세 관측·검증·적용 한계](development/career-long-term-operations-balance-and-navigation-fix-v1.md)와 [소형 CSV](development/career-long-term-operations-balance-and-navigation-fix-v1.csv)를 따른다.


## Career 플레이 동선·임대 동의·목표 대상 교정 V1 (2026-09-09)

원계약 유지 임대의 선수 동의를 새 고용 가격과 분리했고, 시즌 재정 목표는 동일한 대회 참가 집합으로 비교·평가한다. 신규 목표는 해외 운영 조직 준비 뒤 고정하며 LEC Summer 비참가 LR은 경기 목표 중립과 일반 재정 처리를 유지한다. 기존 계약/평가/목표/receipt는 소급 수정하지 않는다.
Career의 현재 업무·Calendar 행동·계약 처리 단계·정확한 Series 승인 결과를 연결했다. 이동은 실행하지 않으며 복귀 후 새로고침도 Career를 유지한다. 집중9회(컴파일 포함)/171건 시도의 경계 실패를 수정했고, 프런트134 PASS/Series87 PASS/build와 격리 브라우저의 거래 응답→기존 Player→Career 왕복을 확인했다. 전체 첫 시도는 클라이언트 연결 단절로 중단됐고, 분리한 프로세스로 재개해286 suites/2,271건 중2,269통과·실패/오류0·기존skip2(wall1,436.35초)로 완료했다. 원본 로그/XML을 보존했으며 정상 전체 이후 후속 집중/추가 전체0회다. 최종 결과와 중단·재개 사유는 [구현·검증 보고서](development/career-play-flow-and-loan-target-fixes-v1.md)를 따른다.
...
정확한 적용 경계, A의12세트와 최종 통합을 구분한 비용 및 검증 결과는 [통합 보고서](development/career-auto-speed-market-economy-and-balance-v2.md)를 따른다.


## Career 준비·수상 조회 교정 및 실제 플레이 속도 V1 (2026-09-09)

관리 경기 A의 준비 표시가 선행 Auto B의 서버 허용 행동을 막지 않도록 교정했다. 준비 표시 닫기와 대상 수명에 따른 정리를 지원한다. 스카우팅 라운드 필터는 기록 조회와 같은 범위로 POG/POM 및 정규시즌 전체 기간상을 표시한다.
Draft 역할 풀의 반복 정의 조회와 즉시 가능한 Continuous 단계의 고정 대기를 줄였으며, 실제 진행 중 발견한 소식함 읽기 스냅샷의 잠금 승격 오류도 교정했다.
동일 저장의 실제 Auto BO3 1개·2일 정산을 전후 각 2회 비교한 최종 화면 대기는 평균 67.257→74.387초였다. **실제 속도 향상은 입증하지 못했고 30% 목표는 미달**이다. 결과/저장 동일성, 측정 한계와 검증 결과는 [실측 보고서](development/career-play-speed-v1.md)에 구분한다.
계획 전체 1회는 284 suites/2,248건 중 2,246 통과·실패/오류 0·기존 skip 2, wall 23분 11.76초로 통과했다. 프런트는 123 PASS/build 성공(Vite 15.09초), 실제 브라우저는 상대 분석→준비→Continuous Auto→관리 경기 정지·화면 반영을 확인했다. 추가 전체는 0회이며 이 테스트 시간은 플레이 성능 향상 수치가 아니다.

## Career 스카우팅·비교·상대 분석 V1 (2026-09-09)

현재 Career 디렉터리·성장·생애주기·공개 시장 견적을 이용한 검색, Career별 관심 저장, 같은 포지션 2~4명 비교와 기존 협상 이동을 연결했다. 최근 승인 경기의 원본 양방향 밴·최종 배정·Hard Fearless 문맥은 V28 부가 원장으로 보존한다. 소식함 리뷰 G1 긴 첫 시즌 최종 성장, G2 예약 Series 준비 이동, G3 과거 화면에서 현재 업무 복귀도 교정했다.
...
[자료 출처·검증 결과와 한계](development/career-scouting-comparison-and-opponent-analysis-v1.md)를 따른다.
...
현재 정수 능력치와 숙련도를 새 경기 입력 및 기존 AI/가격 계산에 공급하며, 이미 시작한
경기·등록·과거 거래는 보존한다. PA는 성장 내부총량의 상한으로만 추가 사용하며 AI 모집과
가격의 선호에는 사용하지 않는다. 피로는 훈련 효율·회복에만 작용하고 경기력 감점은0이다.

V19는 기존 저장의 현재부터 초기화, 완료/날짜 중복 방지와 원본 훈련 요청 복구, 시즌 마감을
저장한다. 관리 구단 기본·선수별 훈련, 타 구단/FA 조회, 정수와 소수 진행도·피로·숙련도·성장
요약을 Career 화면에 제공한다. 소속 변경과 임대·후속 시즌에서도 선수 상태가 이어진다.
...
정책 수치·대표 계산·실제 Auto 및 브라우저 관찰·최종 검증 결과는
[통합 구현 보고서](development/career-player-development-training-proficiency-fatigue-v1.md)에 기록한다.
...
실제 완료 Series의 출전/기회를 시작 시점 계약·운영 소속에 결속하고 역할 약속·만족도·구단
신뢰를 계약 선택에 연결했다. 28일/6 Series 관찰, 주전 70%, 14일 평가를 사용하며 새 경기
근거가 없는 기간은 출전 불이행을 누적하지 않는다. 해외/CL 경기·훈련·성장 근거는 만들지 않는다.

유료 이적과 임대는 기존 56구단 시장·금융·Calendar/명령 receipt 안에서 구단/선수 동의,
적용일 재검사, 양 구단 장부·현재 소속, 임대 급여 분담과 반환을 원자 적용한다. V17과 명시적
recover로 기존 저장은 현재부터 중립 관찰을 시작하며 460명·56구단·82조직, directory와
시작된 Series/국제 등록을 보존한다. 관리 구단 계약 목록은 임대 보낸 원계약 선수도 포함한다.

집중 엔진·저장·production Auto BO3와 실제 브라우저 이적/임대 두 흐름, 원본 요청 응답 소실
복구 및 프런트 63건/build를 확인했다. 최종 전체 회귀는 **265 suites / 총 2,042건 / 통과
2,040 / 실패·오류 0 / 기존 skip 2**, **33분 24초**로 통과했다. 첫 실행의 V17 개수 기대값
1건 교정·집중 통과 후 두 번째 전체로 확인했으며 상세 집계는
[구현·정책·검증 보고서](development/career-playing-time-promises-paid-transfers-and-loans-v1.md)에 기록한다.
다음 순서는 사용자 포텐 준비 후 성장·훈련·챔피언 숙련도·피로다.


## Career 계약 시장 결함 수정·추가 선수 V4 통합 (2026-09-06)

시작 HEAD `7c6f232861fd0486ea0f8021f35ba15e2ab89e5d`에서 계약금 이후 확정 급여의 현금 흐름을
검사하고, 기존 지급 불능 저장은 명시적 미지급금과 다음 연간 배정 우선 정산으로 Calendar를
복구한다. 만료 직전 협상은 기존 종료 다음 날과 공통 결정일 중 늦은 날부터 시작한다.

2026-09-06 V4의 추가 180명 작성값과 상세 정보를 새 Career에 적용했다. 전체 460명·경쟁 팀
56개는 유지하고 연계 육성 조직 추가로 조직은 82개다. 검증된 후보/육성 선수는 유효 게임 계약과
명시적 선발·승격을 통해 사용할 수 있고, 초기 280명 선발과 기존 Career/시작한 Series는 보존한다.
소속 미확인 14명 중 기존 게임 초기 FA 4명만 유지하며 역할 충돌 2명은 별도 검토 대상으로 둔다.

집중 25건과 후속 금융 경계/저장 검증, 프런트 55건 및 build를 통과했다. 실제 브라우저에서
현금 전액 계약금 거절→적정 제안 수락→월말 급여 지급과 날짜/데뷔일 구분을 확인했다.
최종 전체 회귀는 **1회, 29분 6초**, **265 suites / 2,028 tests / 실패·오류 0 / 기존 skip 2**로
통과했다. 결과와 180명별 변경 내역은
[V4 통합 구현 보고서](development/career-market-fixes-and-expanded-player-v4-integration-v1.md)에 기록한다.

## Career 계약·스토브·FA 시장·56개 구단 경쟁 V1 (2026-09-06)

시작 HEAD `41e278ec1819fb29e45c35e7134e0cf66434d4e6`에서 Career 소유 게임 계약·예약 계약,
실제 예산/급여/계약금/해지 비용, 제안·카운터·선수 비교 결정과 55개 AI 경쟁 구단을 연결했다.
V16 이주는 조사 자료를 보존하며 과거 공개 날짜를 소급 방출하지 않는다. 운영 명부 V2는 선발
공백을 허용하고 실제 새 경기에서 필요한 5명과 현재 계약·등록 자격을 확인한다. 유효 등록
대체자가 없는 포지션만 보충등록하며 기존 Series와 원본 등록은 보존한다.

대회 마감 뒤 스토브 운영을 거쳐 12월 31일부터 다음 시즌에 진입한다. 마감 명단과 현재
오프시즌 명단을 분리하고 반복 연도에도 실제 예산·AI 제안 사건을 처리한다. 금액·선호·기간·등록
예외는 현실 규정이나 실제 선수 성격을 뜻하지 않는 게임 정책이다. 화면/복구/최종 검증 기록은
[계약 시장 구현 보고서](development/career-contracts-stove-fa-market-ai-competition-v1.md)에 기록한다.

최종 전체 회귀는 **265 suites / 2,021 tests / 실패·오류 0 / 기존 skip 2**, **27분 25초**로
통과했다. 전체 명령은 1차 기대 날짜 교정, 2차 환경 중단, 복구 후 최종 통과로 세 번 시작했다.
프런트 Career verifier 53건과 production build, 실제 브라우저 협상·응답 소실 복구·다음 시즌
Player Draft, 통합 테스트의 실제 Auto BO3를 확인했다. 준비용 합성 시즌 완료와 실제 경기 실행은
보고서에서 구분한다.
