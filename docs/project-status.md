...
6. Fresh requalification signed payload의 unordered Set을 명시적 canonical order로 바꾸고 raw-tree/deserialize-reserialize digest를 여러 fresh JVM에서 사전 검증한다. 이번 calibration을 consumed ledger에 포함한 새 versioned contract와 비중첩 seed로만 다음 calibration/holdout을 실행하며 같은 seed나 실패 finalizer를 재사용하지 않는다.
